...
(assert (forall 
	((currentvalue Real)
	 ...
	 (controlsignal Real) (i Real) (target Real)
	)

	(=>
		(and
			(<= {{REAL1}} currentvalue) (<= currentvalue {{REAL2}})
            (<= {{REAL3}} target) (<= target {{REAL35}} )
			(= i 0.0)
		)
		( Inv currentvalue error controloutput controlsignal i target))
))
...
(assert (forall 
	((currentvalue Real)
	 ...
	 (controlsignal0 Real) (i0 Real) (target Real)
	)
	...
	(=> 
		(and
			...
			(not (=> 
				(>= i0 {{REAL5}}) 
				(and
					(<= (- 0.0 {{REAL6}}) (- target currentvalue0)) (<= (- target currentvalue0) {{REAL7}})
				)))
		)
		false)
))
...
int main()
{
    ...
    const double R1_MIN = 0.0, R1_MAX = 2000.0;
    const double R2_MIN = 2000.0, R2_MAX = 4000.0;
    const double R3_MIN = 0.0, R3_MAX = 2000.0;
    const double R35_MIN = 2000.0, R35_MAX = 4000.0;
    const double R4_MIN = 0.08, R4_MAX = 0.12;
    const double R5_MIN = 110.0, R5_MAX = 10000.0;
    const double R6_MIN = 0.5, R6_MAX = 1.0;
    const double R7_MIN = 0.5, R7_MAX = 1.0;
    // =================================================

    // Pre-calculate the values for each parameter
    auto r1_vals = linspace(R1_MIN, R1_MAX, 2);
    auto r2_vals = linspace(R2_MIN, R2_MAX, 2);
    auto r3_vals = linspace(R3_MIN, R3_MAX, 1);
    auto r35_vals = linspace(R35_MIN, R35_MAX, 1);
    auto r4_vals = linspace(R4_MIN, R4_MAX, 3);
    auto r5_vals = linspace(R5_MIN, R5_MAX, STEPS);
    auto r6_vals = linspace(R6_MIN, R6_MAX, 2);
    auto r7_vals = linspace(R7_MIN, R7_MAX, 2);

    // Calculate total expected files
    size_t total_files = r1_vals.size() * r2_vals.size() * r3_vals.size() * r35_vals.size() * r4_vals.size() * r5_vals.size() * r6_vals.size() * r7_vals.size();
                        ...
                        for (double r5 : r5_vals)
                        {
                            for (double r6 : r6_vals)
                            {
                                for (double r7 : r7_vals)
                                {
                                    ...
                                    replace_all(content, "{{REAL35}}", std::format("{:.6f}", r35));
                                    ...
                                    std::string filename = std::format(
                                        "p_{:.2f}_{:.2f}_{:.2f}_{:.2f}_{:.4f}_{:.2f}_{:.2f}_{:.2f}.smt2",
